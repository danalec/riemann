 ...
 *
 * @brief   O(1/sqrt(N)) trace error bound
...
static void test_rms_saturation(void) {
  ...
  int Ns[] = {10, 15, 20, 25,  30,  35,  40,  45,  50,   60,
              70, 80, 90, 100, 150, 200, 300, 500, 1000, 2000};
  int nN = 20;
  double prev_rms = 0.0;
  ...
  (void)n_large;  /* diagnostic counter */
  ...
  printf("    - Weighted (N >= 500): %.3f (favoring larger N)\n",
         rms_inf_weighted);
  printf("  Note: RMS spike at N=90-100 (values %.3f, %.3f) is due to boundary "
...
static void test_bound_verification(void) {
  ...
  printf("  N = %d, RMS = %.6f\n\n", N, rms);
  printf("  %-20s  %10s  %10s  %14s  %14s  %10s\n", "Test function", "sigma_h",
         "|(1/N)Delta|", "bound", "actual/bound", "pass?");
  printf("  %-20s  %10s  %10s  %14s  %14s  %10s\n", "---", "---", "---", "---",
         "---", "---");
  ...
  /* h1(E) = E  -> sigma_h = 1 */
  {
    ...
    printf("  %-20s  %10.4f  %10.6f  %14.8f  %14.8f  %10s\n", "h(E)=E", 1.0,
           err, bnd, err / (bnd + 1e-30), (err <= bnd) ? "YES" : "FAIL");
  }
  ...
  /* h2(E) = e^{-0.01E} -> sigma_h = 0.01 (on [14,inf)) */
  {
    ...
    for (int k = 0; k < nz; k++) {
      trH += exp(-0.01 * ev[k]);
      trZ += exp(-0.01 * ZETA_ZEROS[k]);
    }
    double err = fabs(trH - trZ) / nz;
    double bnd = sigma * rms / sqrt((double)nz);
    printf("  %-20s  %10.4f  %10.6f  %14.8f  %14.8f  %10s\n", "h(E)=e^{-0.01E}",
           sigma, err, bnd, err / (bnd + 1e-30), (err <= bnd) ? "YES" : "FAIL");
  }
...
static void test_autocorrelation(void) {
  ...
  double var = 0.0;
  for (int k = 0; k < nz; k++)
    var += (delta[k] - mean_d) * (delta[k] - mean_d);
  var /= nz;
...
static void test_determinant_bound(void) {
  ...
  (void)liH;  /* accumulated but unused (imaginary part of log det) */
  (void)liZ;  /* accumulated but unused (imaginary part of log det) */
...
int main(void) {
  ...
  printf("#  Theorem: |(1/N)[Tr(h(J_N)) - Sigma h(gamma_k)]| <= sigma_h * "
         "RMS(N) / sqrt(N)\n");
  printf("#####################################################################"
         "###\n\n");
  ...
  printf("#  RMS_inf = 0.61 (converging for N >= 500, verified to N=2000 with "
         "2000 zeros).\n");
