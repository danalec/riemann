 ...
 *
 * prove_path_a_determinant.c - Path A: Spectral Determinant -> Trace Formula -> RH
 *
 ...
 *
 * Compile: gcc -O3 -o bin/prove_path_a_determinant.exe src/prove_path_a_determinant.c -lm
 *
...
static double theta_s(double t) {
    ...
    return 0.5 * t * log(t / (2.0 * M_PI)) - 0.5 * t - M_PI / 8.0
           + u / 48.0 + 7.0 * u * u2 / 5760.0
           + 31.0 * u4 / 80640.0 + 127.0 * u4 * u2 / 430080.0;
}
...
static double gram(int n) {
    ...
    for (int i = 0; i < 30; i++) {
        ...
        if (fabs(f) < 1e-14 * (1.0 + fabs(g))) break;
        g -= f / theta_p(g);
    }
    return g;
}
...
static double sturm_find(const double *d, const double *e, int N, int k,
    ...
    for (int iter = 0; iter < 90; iter++) {
        ...
        if (fabs(hi - lo) < 1e-13 * (fabs(lo) + 1.0)) break;
    }
    return 0.5 * (lo + hi);
}
...
static void eigenvalues(const double *a, const double *b, int N, double *ev) {
    ...
    lo -= 5.0; hi += 5.0;
    for (int i = 0; i < N; i++) ev[i] = sturm_find(a, b, N, i, lo, hi);
}
...
static void test_sturm_oscillation(void) {
    ...
    for (int i = 0; i < nE; i++) {
        ...
        /* at each eigenvalue crossing. The Sturm count gives the argument in units of pi. */

        /* Simplified check: p_N(E) has sign (-1)^{N - N_J(E)} for real symmetric J_N */
        ...
        int ok = (fabs(actual_sign - expected_sign) < 0.01 ||
                  fabs(p_re) < 1e-8);

        printf("%8.1f  %12d  %12.4e  %12.4e  %6s\n",
               E, Nj, p_re, p_im,
               ok ? "OK" : "FAIL");
        if (ok) count_correct++; else count_wrong++;
    }
    printf("\nResult: %d/%d correct\n\n", count_correct, nE);
}
...
static void test_spectral_shift(void) {
    ...
    printf("%8s  %8s  %8s  %8s  %12s  %12s\n",
           "E", "N_J", "N_free", "xi_N(E)", "S(E)", "xi_N-S");
    printf("%8s  %8s  %8s  %8s  %12s  %12s\n",
           "---", "---", "---", "---", "---", "---");
    ...
    for (int i = 0; i < nE; i++) {
        ...
        printf("%8.1f  %8d  %8d  %8d  %12.6f  %12.6f\n",
               E, Nj, Nfree, xi_N, S_val, xi_N - S_val);
...
static void test_birman_krein(void) {
    ...
    printf("%10s  %16s  %16s  %16s  %16s\n",
           "tau", "Tr h(J_N)", "Tr h(J_free)", "Diff (BK)", "int h'xi dE");
    printf("%10s  %16s  %16s  %16s  %16s\n",
           "---", "---", "---", "---", "---");
    ...
    for (int ti = 0; ti < nt; ti++) {
        ...
        printf("%10.0f  %16.8f  %16.8f  %16.8f  %16.8f\n",
               tau, tr_J, tr_free, diff_direct, bk_integral);
    }
 ...
 *
 * Tr h(J_N) - Sigma h(gamma_n) -> Sigma_p Sigma_m (log p/p^{m/2}) [E(m log p) + E(-m log p)]
 * ============================================================ */
...
static void test_trace_formula(void) {
    ...
    printf("%12s  %16s  %16s  %16s  %16s\n",
           "t", "Tr-Sigma diff", "Weil RHS", "|Diff-RHS|", "Ratio");
    printf("%12s  %16s  %16s  %16s  %16s\n",
           "---", "---", "---", "---", "---");
    ...
    for (int ti = 0; ti < nt; ti++) {
        ...
        for (int p = 2; p <= pmax; p++) {
            ...
            for (int m = 1; m <= 5; m++) {
                ...
                double hhat_plus = sqrt(M_PI / (2.0 * t))
                    * exp(-(fmlogp - omega) * (fmlogp - omega) / (2.0 * t));
                double hhat_minus = sqrt(M_PI / (2.0 * t))
                    * exp(-(fmlogp + omega) * (fmlogp + omega) / (2.0 * t));
        ...
        printf("%12.6f  %16.8f  %16.8f  %16.8f  %16.6f\n",
               t, diff, weil_rhs, fabs(diff - weil_rhs), ratio);
    }
...
static void test_spectral_determinant(void) {
    ...
    printf("%8s  %16s  %20s\n", "E", "det_N(E)/det0", "Pi(1-E/gamma_k)/Pi(1-E/lambda_k)");
    printf("%8s  %16s  %20s\n", "---", "---", "---");
    ...
    for (int i = 0; i < nE; i++) {
        ...
        for (int k = 0; k < nz; k++) {
            ...
            if (fabs(r_lambda) > 1e-14) prod_ratio *= r_gamma / r_lambda;
        }
...
static void test_determinant_convergence(void) {
    ...
    printf("%8s  %8s  %16s  %16s  %16s\n",
           "N", "E", "det_N(E)/det0", "ratio vs N=100", "|ratio-1|");
    printf("%8s  %8s  %16s  %16s  %16s\n",
           "---", "---", "---", "---", "---");
