 ...
 */
 ...
 /* test_epsilon_paths.c - Test if different epsilon -> 0 paths give different DFT limits
 *
 ...
 *
 * This test checks if different epsilon_k -> 0 sequences give different results.
 * If they do, the result is a fluke. If they all converge to the same limit,
 * the result might be genuine.
 *
 ...
 *
 * For computational feasibility, we truncate at primes p <= P_MAX and m <= M_MAX.
 */
...
static void fejer_dft_complex(double epsilon, int N, double omega, double *re, double *im) {
...
int main(void) {
    ...
    printf("Paper claims: lim_{N -> infinity} S_N^{(0)}(log p) = -i/(2pi sqrt(p))\n");
    printf("Expected real part: ~ -1/(4pi sqrt(p)) ~ -0.1134 for p=2\n\n");
    
    int p_target = 2;
    double omega = log((double)p_target);
    double expected_im = -1.0 / (2.0 * M_PI * sqrt((double)p_target));  /* Imaginary part */
