 ...
 *
 * Uses Gaussian test functions h_sigma(E) = e^{-E^2/(2sigma^2)} e^{iomegaE} in the
 * Guinand-Weil explicit formula. For each sigma > 0, h_sigma is Schwartz class,
 * avoiding the Abel eps-regulator issue entirely.
 *
 * Tests:
 ...
 *
 * Compile: gcc -O3 -o bin/prove_path_b_gaussian.exe src/prove_path_b_gaussian.c -lm
 *
...
/* ============================================================
 * Gaussian-weighted DFT: G(sigma, omega) = Sigma_n e^{-gamma_n^2/(2sigma^2)} e^{iomegagamma_n} S(gamma_n^+)
 * ============================================================ */
static void gaussian_dft(double sigma, double omega,
 ...
 *
 * RHS = Sigma_p Sigma_m (log p/p^{m/2}) * [g_hat(m log p - omega) + g_hat(-m log p - omega)]
 * where g_hat is the Gaussian Fourier transform (peak at 0)
 *
...
static void test_gaussian_dft_convergence(void) {
    ...
    printf("G(sigma, log p) = Sigma_n e^{-gamma_n^2/(2sigma^2)} e^{i log p * gamma_n} S(gamma_n^+)\n");
    printf("As sigma -> infinity: e^{-gamma_n^2/(2sigma^2)} -> 1, recovering the unweighted DFT.\n\n");
    ...
    printf("%10s  %16s  %16s  %16s\n",
           "sigma", "Re G(sigma)", "Im G(sigma)", "|G|^2");
    printf("%10s  %16s  %16s  %16s\n",
           "---", "---", "---", "---");
...
static void test_resonant_isolation(void) {
    ...
    printf("Weil RHS = Sigma (log q/q^{m/2}) * sqrt(2pisigma^2) e^{-sigma^2(m log q - omega)^2/2}\n");
    ...
    printf("%10s  %16s  %16s  %16s  %20s\n",
           "sigma", "Resonant(p=2,m=1)", "Off-diag sum", "Total RHS", "R/O ratio");
    printf("%10s  %16s  %16s  %16s  %20s\n",
           "---", "---", "---", "---", "---");
...
static void test_super_exponential(void) {
    ...
    printf("Gaussian: e^{-sigma^2Delta^2/2} vs Abel: p^{-eps} = e^{-eps log p}\n\n");
    ...
    printf("%10s  %6s  %18s  %18s  %18s\n",
           "sigma", "q", "Gauss g_hat", "Abel q^{-0.01}", "Abel q^{-0.001}");
    printf("%10s  %6s  %18s  %18s  %18s\n",
           "---", "---", "---", "---", "---");
    ...
    for (int i = 0; i < nS; i++) {
        ...
        for (int j = 0; j < np; j++) {
            ...
            printf("%10.0f  %6d  %18.6e  %18.6e  %18.6e\n",
                   s, q, gauss_suppress, abel_1, abel_2);
        }
        if (i < nS - 1) printf("\n");
    }
    printf("\nGaussian suppression is MUCH stronger than Abel.\n");
    printf("For q>>p: Gaussian gives e^{-csigma^2}, Abel gives q^{-eps} ~ 1-eps log q.\n");
...
static void test_regularization_comparison(void) {
    ...
    printf("  1. Gaussian: Sigma e^{-gamma_n^2/(2sigma^2)} e^{iomegagamma_n} S(gamma_n^+)\n");
    ...
    /* Gaussian, large sigma */
    {
        ...
        printf("%16s  %16.10f  %16.10f  %16.10f\n",
               "Gaussian sigma=10^4", im, expected, fabs(im - expected));
    }
    ...
    /* Fejer (direct eps = 0) */
    {
        ...
        printf("%16s  %16.10f  %16.10f  %16.10f\n",
               "Fejer N=1000", norm * im, expected, fabs(norm * im - expected));
    }
    ...
    /* Abel (eps = 0.01, with Fejer window) */
    {
        ...
        double norm = 2.0 / Nfej;
        printf("%16s  %16.10f  %16.10f  %16.10f\n",
               "Abel eps = 0.01+F", norm * sum_im, expected, fabs(norm * sum_im - expected));
    }
...
static void test_gaussian_limit_commute(void) {
    ...
    printf("Does lim_{sigma -> infinity} lim_{N -> infinity} = lim_{N -> infinity} lim_{sigma -> infinity}?\n\n");
...
static void test_double_limit(void) {
    ...
    printf("Both should approach -i/(2*pi*sqrt(2)) ~ -0.11254 as N,sigma -> inf.\n\n");

    double omega = log(2.0);
    double expected = -1.0 / (2.0 * M_PI * sqrt(2.0));

    int configs[][2] = {
        {100, 200}, {200, 500}, {500, 2000}, {1000, 5000}
    };
    int nC = 4;

    printf("%12s  %8s  %12s  %12s  %12s\n",
           "N", "sigma", "Im(DFT)", "Expected", "|Diff|");
    printf("%12s  %8s  %12s  %12s  %12s\n",
           "---", "---", "---", "---", "---");
    ...
    for (int i = 0; i < nC; i++) {
        ...
        printf("%12d  %8.0f  %12.8f  %12.8f  %12.8f\n",
               N, s, im, expected, fabs(im - expected));
    }
    printf("\n");
}
...
int main(void) {
    ...
    printf("  3. Gaussian provides SUPER-EXPONENTIAL decay (e^{-sigma^2Delta^2})\n");
    ...
    printf("e^{-sigma^2(log q - omega)^2}, giving super-exponential off-diagonal\n");
