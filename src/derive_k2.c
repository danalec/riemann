 ...
 *
 * @brief   Second-order correction formula
 * @paper   yamaguchi-rh-2026.tex, §6.4
 * @theorem Lemma II
 * @proof   Higher-order correction terms
 * @step    2
 *
 * derive_k2.c - Second-order correction with S'(gamma_n) term
 *
 * Tests whether adding S'(gamma_n) to the correction formula reduces RMS below
 * 0.009. corr_n = -pi*(S-0.5)/theta' + alpha * S'(gamma_n)
 *
 ...
 * S'(gamma_n) ~ (S(gamma_{n+1}+) - S(gamma_{n-1}+)) / (gamma_{n+1} -
 * gamma_{n-1})
 */
...
int main(void) {
  ...
  double alpha =
      (sum_xy - sum_x * sum_y / count) / (sum_xx - sum_x * sum_x / count);
  ...
  count = N_REF - 1;
  (void)sz;   /* accumulated but unused in Model 3 */
  (void)szz;  /* accumulated but unused in Model 3 */
  (void)sx;   /* accumulated but unused in Model 3 */
  (void)sy;   /* accumulated but unused in Model 3 */
  ...
  for (int n = 1; n < N_REF; n++) {
    ...
    double tp = theta_p(g_prev);
    double s_shifted = S_AT_ZERO[n] - 0.5;
    double corr_model3 = -M_PI * s_shifted / tp + a3 * S_prime_fdiff(n) +
                         b3 * s_shifted * s_shifted;
  ...
  for (int n = 1; n < N_REF; n++) {
    ...
    double corr_model4 =
        -M_PI * s_shifted / tp + a4 * sp_s + b4 * s_shifted * s_shifted;
  ...
  printf("  Best second-order RMS = %.6f\n",
         fmin(fmin(sqrt(sum_sq1 / count), sqrt(sum_sq2 / count)),
              fmin(sqrt(sum_sq3 / count), sqrt(sum_sq4 / count))));
