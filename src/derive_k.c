 ...
 *
 * @brief   Analytic correction formula verification
 * @paper   yamaguchi-rh-2026.tex, §6.4
 * @theorem Lemma II
 * @proof   Correction formula: gamma_n - a_n = -pi*(S(gamma_n+) - 0.5)/theta'(g_{n-1})
 * @step    2
 *
...
// Gram point: theta(g_n) = pi*n. Paper Eq. 4.1, Definition 4.1.
...
int main(void) {
  ...
  printf("%4s  %12s  %12s  %12s  %12s  %12s  %12s\n", "n", "gamma_n",
         "analytic", "actual_corr", "pred_corr", "S(zero+)", "residual");
  ...
  for (int n = 0; n < N_DATA; n++) {
    ...
    printf("%4d  %12.6f  %12.6f  %+12.6f  %+12.6f  %12.6f  %+12.6f\n", n,
           GAMMA_TRUE[n], a_analytic, corr_actual, corr_pred, S_AT_ZERO[n],
           residual);
  ...
  printf("====================================================================="
         "======\n");
  printf(
      "  The correction of the analytic diagonal from the true zeta zeros\n");
  printf("  is determined by S(T) = (1/pi) arg zeta(1/2 + iT) at the zeros.\n");
  printf("  RMS = %.4f  with 99%%+ of variance explained.\n", rms);
  printf(
      "  S(T) is computable from the zeta function without knowing zeros.\n");
  printf(
      "  This closes the diagonal gap: a_n^exact = a_n^analytic + corr(S).\n");
