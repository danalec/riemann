 ...
 *
 * @brief   Heat kernel asymptotics for Gram Jacobi
 * @paper   yamaguchi-rh-2026.tex, §8.3
 * @theorem Theorem I
 * @proof   Weyl density integral verification
 * @step    4
 *
...
static double continuum_heat(double t, double E_min) {
  /* Numerically integrate int_{E_min}^{1000} e^{-tE} * (1/(2pi))*log(E/(2pi))
   * dE */
  int n_quad = 50000;
  double sum = 0;
...
int main(void) {
  ...
  // Heat kernel expansion. Paper Section 7.2, Theorem I.
  /* ==== Part A: Heat kernel trace at multiple t values ==== */
  printf("=== Part A: Heat Kernel Trace ===\n\n");
  printf("  %4s  %8s  %14s  %14s  %14s  %14s  %12s\n", "N", "t", "Tr(e^{-tH})",
         "Tr(e^{-tzeta})", "continuum", "diff(H-zeta)", "ratio");
  printf("  %4s  %8s  %14s  %14s  %14s  %14s  %12s\n", "---", "---", "---",
         "---", "---", "---", "---");

  double t_vals[] = {0.0001, 0.0005, 0.001, 0.002, 0.005,
                     0.01,   0.02,   0.05,  0.1};
  int n_t = 9;
  ...
  for (int ti = 0; ti < 4; ti++) {
    ...
    for (int j = 0; j < n_t; j++) {
      ...
      printf("  %4d  %8.4f  %14.6f  %14.6f  %14.6f  %+14.6f  %12.8f\n", N, t,
             tr_H, tr_z, cont, tr_H - tr_z, (tr_z > 0) ? tr_H / tr_z : 0.0);
    }
    printf("\n");
    free(a);
    free(b);
  ...
  printf("  Theory: Tr(e^{-tH}) ~ (1/(2pit))*(log(1/t) - gamma - log(2pi)) + "
         "O(1)\n");
...

  {
    ...
    printf("\n  Sub-leading structure: Tr(e^{-tH}) = (1/(2pit))*log(1/t) + C/t "
           "+ D + O(t)\n");
    ...
    for (int j = 0; j < 7; j++) {
      ...
      printf("  t=%.5f: C ~ %.6f (tTr=%.6f)  (predicted: "
             "-(gamma+log(2pi))/(2pi) = %.6f)\n",
             t1, C_est, tTr1, -(EULER_GAMMA + log(2.0 * M_PI)) / (2.0 * M_PI));
      if (t2 > 0)
        printf("  t=%.5f: C ~ %.6f (tTr=%.6f)  diff_C=%.6f\n", t2, C2_est, tTr2,
               C_est - C2_est);
    }
  ...
  for (int ti = 0; ti < 4; ti++) {
    ...
    for (int j = 0; j < n_t; j++) {
      ...
      printf("  %10.4f  %14.8f  %14.8f  %+14.8f  %+14.8f\n", t, tr_H, tr_z,
             tr_H - tr_z, (tr_z > 0) ? (tr_H - tr_z) / tr_z : 0.0);
    }
    printf("\n");
  ...
  printf("      ~ (1/(2pi)) * [-1/2sqrt(pi/t) * (gamma/2 + log(2sqrtt) - "
         "log(2))]\n");
...

  {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      double approx = -(0.5 / (2.0 * M_PI)) * sqrt(M_PI / t) *
                      (EULER_GAMMA / 2.0 + log(2.0 * sqrt(t)));

      printf("  %8.4f  %14.6f  %14.6f  %14.6f\n", t, integral, approx,
             integral / (approx + 1e-30));
    }
  }
  ...
  printf("#  KEY RESULT\n");
  printf("#\n");
  printf("#  Tr(e^{-t J_N}) -> Sigma e^{-t gamma_n} + I_Gamma + 2e^{t/4} + "
         "O(t)\n");
  printf(
      "#  as N -> inf, where I_Gamma is the Gamma integral from Stirling.\n");
  printf("#\n");
  ...
  printf("#  Gamma factor contribution from the functional equation of "
         "zeta(s).\n");
  printf("#\n");
